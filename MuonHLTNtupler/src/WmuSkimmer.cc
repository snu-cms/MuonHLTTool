...
class WmuSkimmer : public edm::EDFilter {
public:
  explicit WmuSkimmer(const edm::ParameterSet&);
...
WmuSkimmer::WmuSkimmer(const edm::ParameterSet& iConfig)
...
bool WmuSkimmer::filter(edm::Event& iEvent, const edm::EventSetup&) {
  ...
  if( iEvent.getByToken(t_gen_particle,h_gen_particle) ) {
    ...
    for (; genp != h_gen_particle->end(); genp++) {
      if(abs(genp->pdgId()) == 13 && genp->isHardProcess()==1) {
  ...
  return (pass_pu && found0);  
}

DEFINE_FWK_MODULE(WmuSkimmer);
